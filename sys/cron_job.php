...
if ($config['pro_system'] == 'on') {
    $users_id = $admin::$db->where('type', 'pro_member')->where('time', strtotime("-30 days"), '<')->orderBy('id','DESC')->get(T_TRANSACTIONS, null, array('user_id'));
    $ids = array();
    foreach ($users_id as $key => $value) {
    	$count = $db->where('user_id',$value->user_id)->where('type', 'pro_member')->orderBy('id','DESC')->getOne(T_TRANSACTIONS);
    	if (!empty($count) && $count->time < strtotime("-30 days")) {
	        $ids[] = $value->user_id;
    	}
    }
    if(!empty($ids)) {
