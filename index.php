...
if( ISSET( $context['user'] ) ){
	...
	if($config['subscription_on_signup'] == 'on' && $config['pro_system'] == 'on' && $context['user']['is_pro'] == 0 && !in_array($apph, ['profile','startup','settings','terms'])){
		...
		if (($context['user']['startup_avatar'] == 0 || $context['user']['startup_info'] == 0 || $context['user']['startup_follow'] == 0) && $app != 'startup' && $context['user']['active']) {
			$app      = 'startup';
			$apph     = 'startup';
			header("Location: $site_url/startup");
			exit;
		}
	}
}
