...
if ($action == 'new-image' && IS_LOGGED && ($config['upload_images'] == 'on')) {
	...
	$context['playtube_support'] = "''";
	$context['playtube_link'] = '';
...
else if($action == 'import-post-reels' && IS_LOGGED){
	if ($config['ffmpeg_sys'] == 'on'){
		require_once('sys/libs/ffmpeg-php/vendor/autoload.php');
	...
	if (!empty($_POST['url']) && $config['tiktok_import'] == 'on' && (preg_match("/(http|https):\/\/www.tiktok\.com\/(.*)\/video\/(.*)+/", $_POST['url']) || preg_match("/(http|https):\/\/vt.tiktok\.com\/(.*)+/", $_POST['url']))) {
		$links = GetTiktokVideoDownloadLink($_POST['url']);
		if ($links['status'] == 400) {
			$data = $links;
			header('Content-Type: application/json');
			...
			$upload = array();
			$upload['filename'] = SaveTiktokVideo($links['video_url']);
			if (!empty($upload['filename'])) {
				$media   = new Media();
				$posts   = new Posts();
				$notif   = new Notifications();
	            ...
	            if ($config['ffmpeg_sys'] == 'on'){
	            	...
	            	if (!empty($thumbnail)) {
	            		...
	            		$data['status']  = 500;
						$data['message'] = lang('import_video_error');
	            	}
	            }
			}
			else{
	...
	if (!empty($_POST['embed']) && !empty($_POST['video_id']) && !empty($_POST['url'])) {
		...
		if ($emsrc && $id_val) {
            ...
            $re_data = array(
            	...
            	"link" => $url,
            	"time" => time(),
            	"type" => ($config['tiktok_import'] == 'on' ? 'reels' : $_POST['embed']),
	...
	if ($vl1 && $vl2) {
		...
		if (!empty($user_posts)) {
			$context['posts'] = o2array($user_posts);
			foreach ($context['posts'] as $key => $post_data) {
				if (empty($post_data['post_id']) && !empty($post_data['pid'])) {
					$post_data['post_id'] = $post_data['pid'];
				}
				$context['page'] ='posts';
...
elseif ($action == 'view-likes' && !empty($_GET['post_id'])) {
...
else if($action == 'tiktok_fetch' && !empty($_POST['url'])) {
	$data['status'] = 400;
	$links = GetTiktokVideoDownloadLink($_POST['url']);
	if ($links['status'] == 400) {
		$data = $links;
    ...
    if (Generic::isUrl($_POST['url'])) {
    	include './sys/libs/simple_html_dom.inc.php';
